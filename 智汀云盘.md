...
<b>（1）</b>如果是私人文件，我们访问的接口是http://192.168.22.245:9020/api/plugin/wangpan/resources//?page_size=30&page=1（前面的域名是从智汀授权返回的sa_lan_address，page_size是每页条数，page是页码，也就是域名+api/plugin/wangpan/resources//+分页参数），此接口拿到的数据都是文件夹，不会出现文件，如果出现的话，那就是后端返回数据的问题。在我们获取到私人文件夹之后，我们可以根据<b> is_encrypt </b>这个字段去判断改文件夹是否加密，如果<b> is_encrypt </b>是1的话为加密文件，为0的话则是未加密文件。

<b>（2）</b>如果是共享文件夹，我们访问的接口是http://192.168.22.245:9020/api/plugin/wangpan/shares（前面的域名是从智汀授权返回的sa_lan_address，，也就是域名+api/plugin/wangpan/shares），此接口拿到的数据是共享文件夹，同样也不会出现文件。在我们获取到共享文件夹之后，我们可以根据<b>from_user</b>这个字段判断是在文件夹创建的共享文件夹还是别人共享给我的文件夹，如果<b>from_user</b>未空的话，则是在文件夹管理创建的文件夹，否则的话是别人共享给我的文件夹，而<b>from_user</b>的值就是共享给我的那个人。

<b>（3）</b>文件夹详情，不管是私人文件夹还是共享文件夹，我们访问的接口都是<b>域名+api/plugin/wangpan/resources//+父路径（一级目录没有）+选择的那个文件夹的路径（对应文件实体类的path字段）</b>，比如说我们从接口http://192.168.22.245:9020/api/plugin/wangpan/resources//?page_size=30&page=1获取到的数据有一个名为<b>cloud_disk（对应name字段）和路径为cloud（对应path字段）</b>的文件夹，那么我们进入<b>cloud_disk</b>文件夹时，访问的接口就是http://192.168.22.245:9020/api/plugin/wangpan/resources//cloud?page_size=30&page=1，如果是<b>cloud_disk</b>的文件夹详情路径就是http://192.168.22.245:9020/api/plugin/wangpan/resources//cloud/path?page_size=30&page=1，以此类推。
